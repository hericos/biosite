...
# Quantos dias exibir no “marketplace”
...
<body>
  ...
  <footer>DailyDeals • atualizado automaticamente a partir do WhatsApp Cloud API</footer>
</body>
</html>
"""
...
def download_media(media_id: str) -> tuple[bytes, str]:
    ...
    if not media_url:
        raise RuntimeError("Media URL não veio no response do Graph")
...
def webhook_receive():
    ...
    try:
        ...
        for entry in entries:
            ...
            for ch in changes:
                ...
                for m in messages:
                    wa_from = m.get("from")
                    ts = m.get("timestamp")
                    msg_time = datetime.fromtimestamp(int(ts), tz=timezone.utc) if ts else datetime.now(timezone.utc)
                    ...
                    if mtype == "text":
                        ...
                        img = m.get("image") or {}
                        text = img.get("caption") or ""  # link muitas vezes vem na legenda
                        media_id = img.get("id")
                        if media_id:
                            image_bytes, image_mime = download_media(media_id)

                    # Extrai links do texto/legenda
        ...
        # Não retorne erro p/ Meta (senão ela reenvia em loop)
        print("Erro processando webhook:", ex)
...
def img(item_id: int):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT image_bytes, image_mime FROM whatsapp.deals WHERE id=%s", (item_id,))
            row = cur.fetchone()
...
# Inicializa DB na subida (Render)
init_db()
